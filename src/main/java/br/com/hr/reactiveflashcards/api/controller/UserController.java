...
public class UserController {
    ...
    save(@Valid @RequestBody final UserRequest userRequest) {
        return Mono.just(userRequest)
                ...
                .doFirst(()
                        -> log.info(
                        "Saving a user with follow data {}",
                        userRequest))
    ...
    public Mono<UserResponse>
    update(@PathVariable @Valid @MongoId(message = "userController.id") final String id, @Valid @RequestBody final UserRequest userRequest) {
        return Mono.just(userRequest)
    ...
    public Mono<Void> delete(@PathVariable @Valid @MongoId(message = "userController.id") final String id) {
        return Mono.just(id)
                ...
                .doFirst(()
                        -> log.info(
                        "Deleting a user with follow id {}",
                        id));
    }
}
